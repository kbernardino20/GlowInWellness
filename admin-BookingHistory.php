...
$query = "SELECT * FROM tbl_adminbookinghistory 
          WHERE DATE_FORMAT(STR_TO_DATE(Appointment_date, '%d/%m/%Y'), '%m/%Y') = '$current_month' 
          ORDER BY STR_TO_DATE(Appointment_date, '%d/%m/%Y') DESC";

$filter_message = "Total booking for the month of " . date('F Y') . " is "; // Default filter message
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' || isset($_GET['filter_type'])) {
    ...
    if ($filter_type === 'daily' && !empty($selected_date)) {
        ...
        $query = "SELECT * FROM tbl_adminbookinghistory 
                  ...
                  ORDER BY STR_TO_DATE(Appointment_date, '%d/%m/%Y') DESC";
        $filter_message = "Total booking on " . $selected_date . " is ";
    }
    // Monthly filter
    elseif ($filter_type === 'monthly' && !empty($selected_month)) {
        $selected_month_formatted = date("m/Y", strtotime($selected_month));
        $query = "SELECT * FROM tbl_adminbookinghistory 
                  WHERE DATE_FORMAT(STR_TO_DATE(Appointment_date, '%d/%m/%Y'), '%m/%Y') = '$selected_month_formatted' 
                  ORDER BY STR_TO_DATE(Appointment_date, '%d/%m/%Y') DESC";
        $filter_message = "Total booking for the month of " . date('F Y', strtotime($selected_month)) . " is ";
    }
    // Yearly filter
    elseif ($filter_type === 'yearly' && !empty($selected_year)) {
        $query = "SELECT * FROM tbl_adminbookinghistory 
                  WHERE YEAR(STR_TO_DATE(Appointment_date, '%d/%m/%Y')) = '$selected_year' 
                  ORDER BY STR_TO_DATE(Appointment_date, '%d/%m/%Y') DESC";
        $filter_message = "Total booking for the Year " . $selected_year . " is ";
    }
}
...
$count_query = "SELECT 
                ...
                WHERE DATE_FORMAT(STR_TO_DATE(Appointment_date, '%d/%m/%Y'), '%m/%Y') = '$current_month'"; // Apply the current month filter
    ...
    if ($count > 0) {
        echo "<thead>
                <tr>
                    ...
                    <th style='color: #7E007F; min-width: 200px;'>Name</th>
                    <th style='color: #7E007F; min-width: 200px;'>Appointment Date</th>
                    <th style='color: #7E007F; min-width: 250px'>Service</th>
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<tr>
                    ...
                    <td style='width: 200px;'>
                        <a href='admin-view_booking.php?Ref_No=".urlencode($row['Ref_No'])."&ViewBack=admin-BookingHistory.php' class='settings' title='View Booking' data-toggle='tooltip'><i class='material-icons'>&#xE8F4;</i></a>
                    </td>
                </tr>";
        }
