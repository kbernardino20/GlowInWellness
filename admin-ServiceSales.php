...
$query = "SELECT * FROM tbl_adminbookinghistory 
          WHERE status = '$Stats' 
          AND DATE_FORMAT(STR_TO_DATE(Appointment_date, '%d/%m/%Y'), '%m/%Y') = '$current_month' 
          ORDER BY STR_TO_DATE(Appointment_date, '%d/%m/%Y') DESC
          LIMIT $offset, $records_per_page";  // Added the LIMIT clause here

$total_sales_query = "SELECT SUM(price) as total_sales FROM tbl_adminbookinghistory 
                      WHERE status = '$Stats' 
                      AND DATE_FORMAT(STR_TO_DATE(Appointment_date, '%d/%m/%Y'), '%m/%Y') = '$current_month'";

// Initialize variables for filter labels and filter type
$filter_label = "for the month of " . date("F Y");
$selected_date = $selected_month = $selected_year = '';
$filter_type = 'monthly'; // Set the default filter type to 'monthly';

// Check if a filter was applied from POST or GET (for pagination)
$filter_type = $_POST['filter_type'] ?? $_GET['filter_type'] ?? 'monthly';
$selected_date = $_POST['selected_date'] ?? $_GET['selected_date'] ?? '';
$selected_month = $_POST['selected_month'] ?? $_GET['selected_month'] ?? '';
$selected_year = $_POST['selected_year'] ?? $_GET['selected_year'] ?? '';
...
if ($filter_type === 'daily' && !empty($selected_date)) {
    ...
    $query = "SELECT * FROM tbl_adminbookinghistory 
              WHERE status = '$Stats' 
              AND Appointment_date = '$selected_date_formatted' 
              ORDER BY STR_TO_DATE(Appointment_date, '%d/%m/%Y') DESC
              LIMIT $offset, $records_per_page"; // Added LIMIT here for daily filter
...
elseif ($filter_type === 'monthly' && !empty($selected_month)) {
    ...
    $query = "SELECT * FROM tbl_adminbookinghistory 
              WHERE status = '$Stats' 
              AND DATE_FORMAT(STR_TO_DATE(Appointment_date, '%d/%m/%Y'), '%m/%Y') = '$selected_month_formatted' 
              ORDER BY STR_TO_DATE(Appointment_date, '%d/%m/%Y') DESC
              LIMIT $offset, $records_per_page";  // Added LIMIT here for monthly filter
...
elseif ($filter_type === 'yearly' && !empty($selected_year)) {
    $query = "SELECT * FROM tbl_adminbookinghistory 
              WHERE status = '$Stats' 
              AND YEAR(STR_TO_DATE(Appointment_date, '%d/%m/%Y')) = '$selected_year' 
              ORDER BY STR_TO_DATE(Appointment_date, '%d/%m/%Y') DESC
              LIMIT $offset, $records_per_page";  // Added LIMIT here for yearly filter
...
// Count the total number of filtered records without limit
$count_query = str_replace("LIMIT $offset, $records_per_page", "", $query); // Remove LIMIT for counting
$result_count = $db_connection->query($count_query);
$total_filtered_records = mysqli_num_rows($result_count);

// Execute the main query for pagination
...
<div class="container-xl">
    <div class="table-responsive">
        <div class="table-wrapper">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-7">
                        ...
                        <form method="POST" class="form-inline">
                            ...
                            <!-- Daily Date Picker -->
                            <input type="date" name="selected_date" class="form-control mx-2" id="daily-input" max="<?php echo date('Y-m-d'); ?>" value="<?php echo $selected_date; ?>">
                            <!-- Monthly Input -->
                            <input type="month" name="selected_month" class="form-control mx-2" id="monthly-input" max="<?php echo date('Y-m'); ?>" value="<?php echo $selected_month; ?>">
                            <!-- Year Input -->
                            <input type="number" name="selected_year" class="form-control mx-2" placeholder="Enter Year" min="2000" max="<?php echo date('Y'); ?>" id="yearly-input" value="<?php echo $selected_year; ?>">
            ...
            <!-- Adjust the table styling and remove fixed layout -->
            <table class="table table-striped table-hover" style="width: 100%;">
                ...
                if ($result) {
                    ...
                    if ($count > 0) {
                        echo "<thead>
                                <tr>
                                    <th style='color: #7E007F; min-width: 120px;'>Booking No</th>
                                    <th style='color: #7E007F; min-width: 150px;'>Name</th>
                                    <th style='color: #7E007F; min-width: 100px;'>Appointment Date</th>
                                    ...
                                    <th style='color: #7E007F; width: 100px;'>Action</th>
                                </tr>
                              </thead>
                              <tbody>";
                        ...
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr>
                                    ...
                                    <td style='width: 100px; text-align: center;'>
                                        <a href='admin-view_booking.php?Ref_No=".urlencode($row['Ref_No'])."&ViewBack=admin-ServiceSales.php' class='settings' title='View Order' data-toggle='tooltip'><i class='material-icons'>&#xE8F4;</i></a>
                                    </td>
                                  </tr>";
                        }
                        ...
                        echo "<tr>
                                <td colspan='7'>
                                    <div class='d-flex justify-content-between'>
                                        ...
                                        <div class='pagination d-flex justify-content-end'>"; // Pagination aligned to the right

                        // Modify the pagination links to include the filter parameters
                        if ($current_page > 1) {
                        ...
                        for ($page = 1; $page <= $total_pages; $page++) {
                            if ($page == $current_page) {
                                echo "<span class='btn btn-primary' style='background-color: #7e007f; color: white;'>$page</span> "; // Active page
                            } else {
                                echo "<a href='?page=$page&filter_type=$filter_type&selected_date=$selected_date&selected_month=$selected_month&selected_year=$selected_year' class='btn btn-secondary' style='background-color: white; color: #5b004b;'>$page</a> "; // Inactive pages
                            }
                        }
                        ...
                        echo "<tr><td colspan='7' class='text-center' style='color: #7E007F; font-size: 20px; padding: 50px;'>You do not have any completed appointments</td></tr>";
                    }
                } else {
                    echo "<tr><td colspan='7' class='text-center' style='color: #7E007F; font-size: 20px; padding: 50px;'>You do not have any completed appointments</td></tr>";
                }
